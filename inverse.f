        function gel(u,nc,mc,B,D,J,sn,cn,dn) 
        real*8 gel,nc,mc,u,B,D,J,sn,cn,dn,n,m
        integer icase
        common /icase/icase
        n=1.d0-nc
        m=1.d0-mc
        ...
        function gel1(u,nc,mc,B,D,J,sn,cn,dn,B1,D1,J1,sn1,cn1,dn1)
        real*8 gel1,nc,mc,u,B,D,J,sn,cn,dn,B1,D1,J1,sn1,cn1,dn1,n,m
        integer icase
        common /icase/icase
        n=1.d0-nc
        m=1.d0-mc
      ...
      function nigel(nc,mc,cB,cD,cJ,uTOL,fTOL,gel,gel1,B,D,J,sn,cn,dn,
     *              f)
      implicit real*8 (a-h,j-z)
      real*8 nigel
      integer i
      external gel
      real*8 aigel,gel,gel1
      m=1.d0-mc
      n=1.d0-nc
      h=n*nc*(mc-nc)
      ...
      function aigel(nc,mc,cB,cD,cJ,uTOL,fTOL,gel,B,D,J,sn,cn,dn,f)
      implicit real*8 (a-h,j-z)
      real*8 aigel
      integer i
      real*8 gel
      ...
      do i=1,60
         ...
         DH=0.5d0*(DH-W)
         t=W/(1.d0-n*(y-yH*v))
         JH=0.5d0*(JH-uatan(t,h))
         u=uL+uH
         xi=cnL*cnH
         eta=snL*snH
         zeta=dnL*dnH
         nu=1.d0/(1.d0-m*yL*yH)
         cn=(xi-eta*zeta)*nu
         ...
         function uatan(t,h)
         real*8 uatan,t,h,z,r,y,A3,A5,A7,A9
         ...
         elseif(z.lt.0.d0) then
                 ...
                 r=sqrt(-h)
                 y=r*t
                 uatan=log((1.d0+y)/(1.d0-y))*0.5d0/r
         endif
         return
         end
         ...
         D2=B2*mp
         D3=B3*(2.d0*m2p+13.d0*m)
         D4=B4*(m3p+30.d0*mm2)
         D5=B5*(2.d0*m4p+251.d0*mm3+876.d0*m2)
         D6=B6*(2.d0*m5p+1018.d0*mm4+9902.d0*m2m3)
         J2=B2*3.d0
         J3=B3*(30.d0*mp-45.d0*n)
         J4=B4*(63.d0*m2p+252.d0*m-315.d0*mp*n+315.d0*n2)
         J5=B5*(510.d0*m3p+5850.d0*mm2-(6615.d0*m2p+21735.d0*m)*n+
     *                                 18900.d0*mp*n2-14175.d0*n3)
     ...
     *          1164240.d0*m)*n2-779625.d0*mp*n3+467775.d0*n4)
         ...
         function bigel(nc,mc,pTOL,fTOL,gel,B,D,J,sn,cn,dn,f)
                 implicit real*8 (a-h,j-z)
                 real*8 bigel
                 integer i
                 real*8 gel
